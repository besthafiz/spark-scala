...
import org.apache.spark.sql.functions.{col, concat_ws, dense_rank, rank}

object WindowDenseRank {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val win2 = Window.orderBy(col("Salary").desc)
    val deptSalaryDF = finalDf.select(col("*"), dense_rank().over(win2
    ).as("rank"))
