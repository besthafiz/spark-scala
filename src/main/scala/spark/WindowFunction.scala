...
import org.apache.spark.sql.functions.{col, concat_ws, rank}
...
//  First/last occurrence per group
  def main(args: Array[String]): Unit = {
    ...
    val deptData=Seq((1,123,"IT",20000),(2,345,"IT",20000),(3,12,"HR",30000),(4,34,"HR",40000))
    ...
    finalDf.show(false)
    val win=Window.partitionBy("DeptName").orderBy(col("Salary").desc)
    val deptSalaryDF=finalDf.select(col("*"),rank().over(win).as("rank"))
